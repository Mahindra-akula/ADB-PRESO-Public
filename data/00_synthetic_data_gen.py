...
# Generates synthetic POS data for 100 stores × 50 SKUs × 30 days = 150,000 rows
# PySpark only — no pandas
...
spark.sql(f"CREATE VOLUME IF NOT EXISTS {CATALOG}.{BRONZE_SCHEMA}.raw_pos")
...
print(f"stores: {stores_df.count()} rows written")
...
print(f"skus: {skus_df.count()} rows written")

# COMMAND ----------
# ── Fact: POS transactions — 100 stores × 50 SKUs × 30 days = 150,000 rows ────
end_date   = datetime.date.today()
start_date = end_date - datetime.timedelta(days=N_DAYS - 1)

dates_df = spark.sql(f"""
    SELECT explode(sequence(DATE('{start_date}'), DATE('{end_date}'), INTERVAL 1 DAY)) AS transaction_date
""")
...
pos_raw = (
    ...
    .withColumn("seasonal_multiplier",
        F.when(F.col("day_of_week").isin(1, 7), 1.25)   # weekend boost
         .when(F.col("day_of_week") == 2, 0.90)           # Monday dip
         .otherwise(1.0))
    .withColumn("is_stockout", F.rand(seed=99) < 0.12)   # simulate 12% stockout rate
    .withColumn("qty_sold",
...
# COMMAND ----------
# Write raw Parquet to Volume — DLT pipeline reads from here
pos_raw.write.mode("overwrite").parquet(f"{VOLUME_PATH}/pos_transactions/")
print(f"POS transactions written: {pos_raw.count()} rows → {VOLUME_PATH}/pos_transactions/")
